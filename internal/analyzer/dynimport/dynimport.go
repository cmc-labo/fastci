// Package dynimport implements a safety net for import()/require() calls in
// JS/TS source whose target esbuild can't resolve statically. It's shared by
// every fastci analyzer that uses esbuild as its resolver (jestanalyzer,
// vitestanalyzer) - see the package doc on either for the general shape of
// the problem. Two cases matter:
//
//   - A fully opaque call, e.g. import(pick()): esbuild emits no diagnostic
//     and no edge at all; invisible to metafile-based edge extraction.
...
//     as a single "external" entry, which callers' `if imp.External {
//     continue }` metafile handling silently drops.
//
// Both are handled the same way: ScanFiles finds these call sites by a
// lightweight source-text scan (esbuild's public Go API doesn't expose a
// parse tree), and NeutralizerPlugin rewrites each template-literal call's
// argument to an inert bare specifier via an OnLoad plugin *before*
// esbuild's parser ever sees the original text - a bare specifier resolves
// like a node_modules import under the Packages: PackagesExternal option
// callers already use, i.e. external, zero errors, no glob classification.
// This makes the fatal-error case structurally impossible rather than
// caught-and-suppressed, and means the dropped-edge case never reaches
// esbuild's resolver at all: the caller instead resolves a safe superset of
// edges itself (every file under the matched directory) and marks fully
// opaque calls (and unmatched directory prefixes) so impact analysis always
// includes them.
package dynimport
...
// Scan is the result of scanning one source file for import()/require()
// call sites whose argument isn't a plain string/template literal esbuild
// can already resolve on its own.
type Scan struct {
...
// ScanSites scans src (a JS/TS/JSX/TSX source file) for import()/require()
// call sites and classifies each one. It's a lightweight hand-rolled scan,
// not a real parser, so it's deliberately biased toward false positives
// over false negatives: e.g. a plain method call named import or require
// (foo.import(x)) gets flagged even though it isn't a dynamic import, but a
// genuinely unresolvable call is never missed.
func ScanSites(src []byte) Scan {
	var scan Scan
	n := len(src)
	for i := 0; i < n; {
		c := src[i]
...
func classifyDynamicCall(src []byte, argStart int, scan *Scan) int {
	n := len(src)
	i := argStart
...
// importÖnem) isn't misread as the bare keyword - see ScanSites.
...
// RewrittenFile is the neutralized source NeutralizerPlugin substitutes for
// a file with one or more TemplateCall sites.
...
// LoaderForExt returns the esbuild loader to use for a rewritten file with
// the given extension (dot included), defaulting to plain JS for anything
// unrecognized.
...
// NeutralizerPlugin serves the pre-rewritten contents in rewrites (keyed by
// absolute file path, matching OnLoadArgs.Path for the "file" namespace) in
// place of the real file contents, so esbuild's parser never sees the
// original template-literal dynamic import argument that would otherwise
// crash the build or produce a silently-dropped edge - see the package doc
// above. Every other file falls through to normal loading.
func NeutralizerPlugin(rewrites map[string]RewrittenFile) api.Plugin {
	return api.Plugin{
		Name: "fastci-dynamic-import-neutralizer",
		Setup: func(build api.PluginBuild) {
			build.OnLoad(api.OnLoadOptions{Filter: `\.([cm]?[jt]sx?)$`}, func(args api.OnLoadArgs) (api.OnLoadResult, error) {
...
// ScanFiles scans every file in files (absolute paths) for dynamic
// import()/require() call sites. It returns the set of files with at least
// one opaque call, the neutralized contents to feed esbuild for every file
// with at least one template call (keyed by absolute path, ready for
// NeutralizerPlugin), and the template calls found per file (for the caller
// to resolve into real edges once every file is a known graph node).
func ScanFiles(files []string) (opaqueFiles map[string]bool, rewrites map[string]RewrittenFile, templateCallsByFile map[string][]TemplateCall, err error) {
	...
	for _, f := range files {
		...
		if readErr != nil {
			return nil, nil, nil, fmt.Errorf("dynimport: reading %s: %w", f, readErr)
		}
		scan := ScanSites(src)
		if scan.Opaque {
			opaqueFiles[f] = true
